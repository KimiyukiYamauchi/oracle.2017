...
1. employees表から職種にREPが含まれる従業員の給与の平均、  
最高、最低、合計を表示(1129.1.sql)
1. employees表から入社が最も古い従業員の従業員番号、姓、  
入社日を表示(1129.2.sql)
1. employees表から入社が最も新しい従業員の従業員番号、姓、  
入社日を表示(1129.3.sql)
1. employees表から職種及び職種別の人数を表示(1129.4.sql)
1. employees表とdepartments表から従業員の従業員番号、姓、  
部門番号、部門名を表示。この時、配属が未確定の従業員  
及び誰も配属されていない部門も表示する(1129.5.sql)

## 11/28
### 「hr」ユーザ

1. employees表から部門番号90の従業員の従業員番号、姓、  
入社から現在までの週数(小数点以下は切り捨て、列別名:WEEKS)  
を表示、従業員番号でソート(1128.1.sql)
1. employees表から部門番号90の従業員の従業員番号、姓、  
入社から現在までの月数(小数点以下は四捨五入、列別名:TENURE)  
を表示、従業員番号でソート(1128.2.sql)
1. employees表から雇用月数が120ヶ月未満の従業員の従業員番号、  
雇用日、6ヶ月審査日(列別名:REVIEW)、雇用日後の最初の金曜日、  
雇用月の最後の日を表示、従業員番号でソート(1128.3.sql)
1. employees表から従業員の従業員番号、姓、入社日を表示。  
入社日は[17 Semptember 2008]の形式で、余計なスペースは  
表示しない。従業員番号でソート(1128.4.sql)
1. employees表から従業員番号、姓、年収((給与+給与×歩合)×12
但し、  
歩合がない場合は0とみなす、列別名:AN_SAL)を表示、  
従業員番号でソート(1128..sql)
...
1. employees表から部門番号90の従業員の従業員番号、姓、職種  
部門番号を表示、従業員番号でソート(1122.1.sql)
1. employees表から姓がWhalenの従業員の従業員番号、姓、職種  
を表示(1122.2.sql)
1. employees表から2008年入社の従業員の従業員番号、姓、入社日  
を表示、従業員番号でソート(1122.3.sql)
1. employees表から給与が2500以上3500以下の従業員の従業員番号、  
姓、給与を表示、従業員番号でソート(1122.4.sql)
1. employees表から職種にREPが含まれる従業員の従業員番号、姓、  
職種を表示、従業員番号でソート(1122.5.sql)

## 11/21
### 「hr」ユーザ

1. departments表のすべての列の表示、部門番号でソート(1121.1.sql)
1. employees表から姓と給与と給与に300ドル加えたものを  
表示、従業員番号でソート(1121.2.sql)
1. employees表から姓(列別名:Name)、給与の12倍(列別名:Annual Salary)  
を表示、従業員番号でソート(1121.3.sql)
1. employees表から「姓 is a 職種.」の形式(列別名:Employee Details)  
で表示、従業員番号でソート(1121.4.sql)
1. employees表から職種の種類(列別名:Job)を表示、職種でソート  
(1121.5.sql)
...
1. TITLE表にprice列(NUMBER(8,2))を追加して、ビデオの購入価格  
の列を作成
1. ビデオの価格列に上記表のデータを追加するスクリプトを作成(1116.1.sql)
1. 上記スクリプトを利用して、購入価格を追加
1. 各顧客のレンタル履歴を顧客名でソートして、表示するスクリプト  
を作成(1116.2.sql)
    - 顧客名(last_name)
    - タイトル
    - レンタル開始日
