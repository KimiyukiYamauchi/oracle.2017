...
- 正規化
    - 第1正規形 => 主キーに対して複数回現れるデータ項目がない  
    (配列がないこと)、主キーに設定されている列で列が特定できる
    - 第2正規形 => 第1正規形であること、かつ主キーに依存している  
    (部分関数従属がない)こと
    注文明細(主キー => 注文番号、製品番号)、  
    商品(主キー => 製品番号、製品名)
    - 第3正規形 => 主キーでない属性に依存する属性がない  
    (推移的関数従属性がない)こと
    社員表(主キー => 社員番号、部門番号)、  
    部門表(主キー => 部門番号、部門名)
...
- グループ関数の意味
- グループ関数の使い方
    - max(列名), min(列名), avg(列名), sum(列名), count(列名)  
     => null値は無視する
    - count(*) => 列のすべての値がnullでも無視しない
    - count, min, maxは日付値、文字列値にも使える
- GROUP BY
    - whereより下、order byよりは上
    - select句にはgroup byで指定した列かグループ関数のみ
    - group by句で列別名は不可
- HAVING
    - GROUP BYとHAVINGは順不同
    - 集計した行に対する選択に使用(条件にグループ関数がある)
    - WHEREは不可

### 6 複数の表からのデータの取り出し
- 等価結合 => 結合する表で、共通する列を結合条件とする
    - 自然結合
- 非等価結合
- 自己結合
- 外部結合
- 直積(デカルト積) => クロス結合、結合条件を指定しない
- 標準の方法とOracle独自には性能差はない
